...
describe('getGraphQLDocPath', () => {
  it('should return com/amplify/generated/graphql when used in android project', () => {
    ...
    expect(getGraphQLDocPath('android', graphQLDirectory)).toEqual(
      join('app', 'src', 'main', 'graphql', 'com', 'amazonaws', 'amplify', 'generated', 'graphql'),
    );
  });
  ...
  it('should return default folder when frontend is not android and include path not defined', () => {
    ...
    expect(getGraphQLDocPath('javascript', graphQLDirectory)).toEqual(join('src', 'graphql'));
  });
