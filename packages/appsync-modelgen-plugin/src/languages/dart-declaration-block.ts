...
export class DartDeclarationBlock {
  ...
  public get string(): string {
    ...
    const methods = this._methods.length
      ...
      : null;
    const blocks = this._blocks.length ? this._blocks.map((b) => (b._kind ? indentMultiline(b.string) : b.string)).join('\n\n') : null;
  ...
  private printMethod(method: ClassMethod): string {
    const signature = [
      ...
      .filter((f) => f)
      .join(' ');
    const args = !method.flags.isGetter ? `(${method.args.map((arg) => this.printMember(arg)).join(', ')})` : '';
    const comment = method.comment ? transformComment(method.comment) : '';
    const annotations = method.annotations.map((a) => `@${a}\n`).join('');
    const implementation = method.flags.isBlock ? [' {', indentMultiline(method.implementation), '}'].join('\n') : method.implementation;
    return [comment, annotations, signature, args, implementation].join('');
  }
}
