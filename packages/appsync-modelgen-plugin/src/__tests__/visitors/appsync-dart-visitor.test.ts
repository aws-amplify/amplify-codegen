...
const getVisitor = (
  ...
  const visitor = new AppSyncModelDartVisitor(
    builtSchema,
    { directives, target: 'dart', scalars: DART_SCALAR_MAP, enableDartNullSafety, improvePluralization: improvePluralization },
...
describe('AppSync Dart Visitor', () => {
  describe('Model Directive', () => {
    ...
    `;
      const visitor = getVisitor(schema, null, CodeGenGenerateEnum.code, false, true);
      const generatedCode = visitor.generate();
      expect(generatedCode).toMatchSnapshot();
    });
  });
