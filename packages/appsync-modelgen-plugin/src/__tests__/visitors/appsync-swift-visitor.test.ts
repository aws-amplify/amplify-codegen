  ...
  describe('Use Improved Pluralization', () => {
    ...
    beforeEach(() => {
      wishSchema = /* GraphQL */ `
        ...
        type Wishes @model @key(name: "byUsers", fields: ["idUser"]) {
          id: ID!
          date: AWSDateTime!
          idUser: ID!
          ev: ID!
        ...
        type Wish @model @key(name: "byUsers", fields: ["idUser"]) {
          id: ID!
          date: AWSDateTime!
          idUser: ID!
          ev: ID!
    ...
    it('Should work with potentially pluralized collision', () => {
      const visitor = getVisitor(wishSchema, 'Wish', CodeGenGenerateEnum.code, {
        improvePluralization: true,
      });
      const generatedCode = visitor.generate();
      expect(generatedCode).toMatchSnapshot();

      const metadataVisitor = getVisitor(wishSchema, 'Wish', CodeGenGenerateEnum.metadata, {
        improvePluralization: true,
      });
      const generatedMetadata = metadataVisitor.generate();
      expect(generatedMetadata).toMatchSnapshot();

      const customTypeVisitor = getVisitor(wishSchema, 'Wishes', CodeGenGenerateEnum.code, {
        improvePluralization: true,
      });
      expect(customTypeVisitor.generate()).toMatchSnapshot();
    });
