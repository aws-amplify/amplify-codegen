...
> extends BaseVisitor<TRawConfig, TPluginConfig> {
  ...
  protected computeVersion(): string {
    ...
    Object.values({ ...this.modelMap, ...this.nonModelMap }).forEach((obj: CodeGenModel) => {
      ...
      const directives = (this.config.usePipelinedTransformer || this.config.transformerVersion === 2) ? obj.directives.filter(dir => dir.name === 'primaryKey' || dir.name === 'index') : obj.directives.filter(dir => dir.name === 'key');
      const fields = obj.fields
        .map((field: CodeGenField) => {
          // include only connection field and type
          const fieldDirectives = (this.config.usePipelinedTransformer || this.config.transformerVersion === 2) ? field.directives.filter(field => field.name === 'hasOne' || field.name === 'belongsTo' || field.name === 'hasMany' || field.name === 'manyToMany') : field.directives.filter(field => field.name === 'connection');
          return {
            name: field.name,
