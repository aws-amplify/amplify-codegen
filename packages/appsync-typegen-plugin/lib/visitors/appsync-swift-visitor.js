...
class AppSyncSwiftVisitor extends appsync_visitor_1.AppSyncModelVisitor {
  ...
  generateSchema() {
    ...
    Object.values(this.getSelectedModels())
      ...
      .forEach(model => {
        const schemaDeclarations = new swift_declaration_block_1.SwiftDeclarationBlock()
          .asKind('extension')
  ...
  generateModelSchema(name, model, extensionDeclaration) {
    ...
    extensionDeclaration.addProperty(
      'schema',
      '',
      `defineSchema ${visitor_plugin_common_1.indentMultiline(closure).trim()}`,
      'public',
      ...
      (_a = directive.arguments) === null || _a === void 0
        ...
        : _a.rules.forEach(rule => {
            ...
            switch (rule.allow) {
              ...
              default:
                warn_1.printWarning(
                  `Model ${model.name} has auth with authStrategy ${rule.allow} of which is not yet supported in DataStore.`,
                );
                return;
            }
