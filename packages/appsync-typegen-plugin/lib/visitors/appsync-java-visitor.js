...
class AppSyncModelJavaVisitor extends appsync_visitor_1.AppSyncModelVisitor {
  ...
  generateClassLoader() {
    ...
    result.push(
      visitor_plugin_common_1.transformComment(ts_dedent_1.default` Contains the set of model classes that implement {@link Model}
    interface.`),
    );
    ...
    loaderClassDeclaration.addClassMember('amplifyGeneratedModelInstance', java_config_1.LOADER_CLASS_NAME, '', [], 'private', {
      static: true,
    });
  ...
  generateQueryFields(model, field, classDeclarationBlock) {
    ...
    const fieldName =
      field.connectionInfo && field.connectionInfo.kind === process_connections_1.CodeGenConnectionType.BELONGS_TO
  ...
  generateStepBuilderInterfaces(model, isModel = true) {
    ...
    const interfaces = requiredInterfaces.map((field, idx) => {
      ...
      interfaceDeclaration.withBlock(
        visitor_plugin_common_1.indent(`${this.getStepInterfaceName(returnType)} ${methodName}(${argumentType} ${argumentName});`),
      );
  ...
  generateBuilderClass(model, classDeclaration, isModel = true) {
    ...
    builderClassDeclaration.addClassMethod(
      ...
      visitor_plugin_common_1.indentMultiline(buildImplementation.join('\n')),
      undefined,
      [],
      'public',
      {},
    ...
    @throws IllegalArgumentException Checks that ID is in the proper format`;
      builderClassDeclaration.addClassMethod(
        ...
        visitor_plugin_common_1.indentMultiline(idBuildStepBody),
        [{ name: 'id', type: 'String' }],
        [],
        'public',
        {},
  ...
  generateCopyOfBuilderClass(model, classDeclaration, isModel = true) {
    ...
    const invocations =
      stepBuilderInvocation.length === 0
        ? ''
        : ['super', visitor_plugin_common_1.indentMultiline(stepBuilderInvocation.join('\n')).trim(), ';'].join('');
  ...
  generateCopyOfBuilderMethod(model, classDeclaration) {
    const args = visitor_plugin_common_1
      .indentMultiline(
  ...
  generateEqualsMethod(model, declarationBlock) {
    ...
    const propCheck = visitor_plugin_common_1
      .indentMultiline(
        this.getNonConnectedField(model)
          .map(field => {
    ...
    declarationBlock.addClassMethod(
      'equals',
      'boolean',
      visitor_plugin_common_1.indentMultiline(body.join('\n')),
  ...
  generateHashCodeMethod(model, declarationBlock) {
    ...
    declarationBlock.addClassMethod('hashCode', 'int', visitor_plugin_common_1.indentMultiline(body).trimLeft(), [], [], 'public', {}, [
      'Override',
    ]);
  }
  ...
  generateBuilderMethod(model, classDeclaration) {
    ...
    classDeclaration.addClassMethod(
      'builder',
      returnType,
      visitor_plugin_common_1.indentMultiline(`return new Builder();`),
      [],
      [],
      'public',
      { static: true },
  ...
  generateAuthRules(authDirectives) {
    ...
    authDirectives.forEach(directive => {
      var _a;
      (_a = directive.arguments) === null || _a === void 0
        ? void 0
        : _a.rules.forEach(rule => {
            var _a;
            var _b;
            const authRule = [];
            switch (rule.allow) {
              ...
              case process_auth_1.AuthStrategy.groups:
                ...
                if (rule.groups) {
                  authRule.push(
                    `groups = { ${(_a = rule.groups) === null || _a === void 0 ? void 0 : _a.map(group => `"${group}"`).join(', ')} }`,
                  );
                } else {
            ...
            authRule.push(
              `operations = { ${
                (_b = rule.operations) === null || _b === void 0 ? void 0 : _b.map(op => operationMapping[op]).join(', ')
              } }`,
            );
  ...
  generateJustIdMethod(model, classDeclaration) {
    ...
    const initArgs = visitor_plugin_common_1.indentMultiline(
      ['id', ...new Array(this.getNonConnectedField(model).length - 1).fill('null')].join(',\n'),
    );
