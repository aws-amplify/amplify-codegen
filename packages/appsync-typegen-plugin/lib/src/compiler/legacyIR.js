...
class LegacyIRTransformer {
  ...
  transformIR() {
    ...
    for (const [operationName, operation] of Object.entries(this.context.operations)) {
      ...
      const { sourceWithFragments, operationId } = generateOperationId_1.generateOperationId(
        operation,
  ...
  transformSelectionSetToLegacyIR(selectionSet) {
    ...
    const inlineFragments = typeCase.variants.flatMap(variant => {
      ...
      if (selectionSet.possibleTypes.every(type => variant.possibleTypes.includes(type)) && fields.length < 1) return undefined;
      const fragmentSpreads = this.collectFragmentSpreads(selectionSet, variant.possibleTypes).map(
